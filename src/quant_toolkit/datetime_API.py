...
# Helper functions related to datatime objects
def get_holiday_list() -> List[datetime.date]:
...
def validate_holiday(dt: datetime.date) -> datetime.date:
...
def validate_weekday(dt: datetime.date) -> datetime.date:
...
def validate_expiry(dt: datetime.date) -> datetime.date:
    ...
    """
    holiday_dates = get_holiday_list()
...
def find_next_weekday(
    ...
    """
    Find the date of the specified weekday in the next week.
    """

    return find_current_weekday(today_date, target_day) + datetime.timedelta(days=7)
...
class FNOExpiry:
    ...
    def finnifty_current_month_fut_expiry(
        self,
        today_date: datetime.date = datetime.date.today(),
    ) -> str:
        today_date = validate_holiday(today_date)
        today_date = validate_weekday(today_date)
        ...
        expiry_date = validate_expiry(expiry_date)
    ...
    def finnifty_next_month_fut_expiry(
        self,
        today_date: datetime.date = datetime.date.today(),
    ) -> str:
        today_date = validate_holiday(today_date)
    ...
    def nifty_current_week_opt_expiry(
        ...
        expiry_date = find_current_weekday(today_date)
        expiry_date = validate_expiry(expiry_date)
        if expiry_date < today_date:
            expiry_date = find_next_weekday(today_date)
        _year, _month, _day = expiry_date.year, expiry_date.month, expiry_date.day
        return (
            "NSE:NIFTY"
            + str(_year)[-2:]
...
if __name__ == "__main__":
    futures = ["NSE:COROMANDEL-FUT", "NSE:INDHOTEL-FUT", "NSE:HINDPETRO-FUT"]
    for i in futures:
        print(_main(i))
    main()
