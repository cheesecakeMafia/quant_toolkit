...
def validate_params(func: Callable[P, T]) -> Callable[P, T]:
    """Validate function parameters against their type annotations.
    
    Performs runtime type checking based on function type hints. Supports:
    - Basic types (int, str, float, bool, etc.)
    - Union types (int | str)
    - Optional types (Optional[T] or T | None)
    - Skips validation for Any type
    
    Args:
        func: Function to wrap with parameter validation
    
    Returns:
        Wrapped function with parameter validation
    
    Raises:
        TypeError: When parameter type doesn't match annotation
        ValueError: When required parameter is None
    
    Example:
        @validate_params
        def process_data(value: int, name: str | None = None) -> float:
                ...
                if hasattr(expected_type, "__origin__"):
                    ...
                    if expected_type.__origin__ is Union:
                        ...
                        if any(isinstance(value, t) for t in type_args):
                            continue
                        expected_type = (
                            type_args[0] if len(type_args) == 1 else expected_type
                        )
...
def time_logger(func: Callable[P, T]) -> Callable[P, T]:
    ...
    Prints execution time to stdout with automatic unit selection:
    ...
    Example:
        ...
        def expensive_operation():
            time.sleep(0.5)
        # Output: expensive_operation took 500.00ms
    """
...
def slow_down(
    _func: Optional[Callable[P, T]] = None, *, delay: float = 1.0
...
def memoize(func: Callable[P, T]) -> Callable[P, T]:
    ...
    wrapper.cache_clear = lambda: (
        cache.clear(),
        setattr(wrapper, "hits", 0),
        setattr(wrapper, "misses", 0),
    )
...
def singleton(cls: type[T]) -> type[T]:
    ...
    First instantiation arguments are used; subsequent calls ignore arguments.
    
    Args:
        cls: Class to make singleton
    
    Returns:
        Function that returns the single instance
    ...
    Example:
        ...
        class DatabaseConnection:
            def __init__(self, host="localhost"):
                self.host = host
                self.connection = connect_to_db(host)
        
        # All calls return the same instance
        db1 = DatabaseConnection("server1")
        db2 = DatabaseConnection("server2")  # Still returns instance with "server1"
